...
# Path to the resource config file
...
# with open("/opt/ml/input/config/inputdataconfig.json", 'r') as f:
#     data_config = json.load(f)

print(f" PROCESSING {os.listdir('/opt/ml/processing/')}")
print(f" ROOT {os.listdir('/opt/ml/config/')}")
print(f" TRAIN {os.listdir('/opt/ml/processing/input/train/')}")
print(f" INPUT {os.listdir('/opt/ml/processing/input/')}")
# Extract the required information
current_host = resource_config['current_host']
hosts = resource_config['hosts']

# Set the environment variables
os.environ['SM_CURRENT_HOST'] = current_host
os.environ['SM_HOSTS'] = json.dumps(hosts)  # Encode the list as a JSON string
os.environ['SM_MODEL_DIR'] = "/opt/ml/processing/output/"
# Optional: Print the set environment variables to verify
print(f"SM_CURRENT_HOST: {os.environ['SM_CURRENT_HOST']}")
print(f"SM_HOSTS: {os.environ['SM_HOSTS']}")
...
def run_with_args(args):
    ...
    if not (args.use_dask == "True" or len(args.hosts) > 1):  # this corresponds to single instance training
        ...
        X_train, y_train, X_val, y_val = data_prep.prepare_data(
            ...
            content_type="text/csv" #utils.get_content_type(input_data_config=input_data_config),
        )
        ...
        if current_host == main_host:
            with Client(constants.SCHEDULER_IP_TEMPLATE.format(ip=scheduler_ip)) as client:
                try:
                    ...
                    print(f"CAT {categorical_feature}")
                    # booster = utils.get_pretrained_model(pretrained_model_dir=args.pretrained_model)
                    ...
                    print(f"MLFOW LOGGED")
                    dask_model = lgb.DaskLGBMClassifier(**params)
                    print(f"DASK {dask_model}")
                    ...
                    logging.info("TRAININGING Beginning training")
